...
class NodeCluster:
    ...
    def __init__(self, node, dataset=None, adj_list=None, gen_hierarchies=None):
        ...
        self._genRangeFeatures = {
            ...
            'BloodPressure': [self._dataset[node]['BloodPressure'], 
                              self._dataset[node]['BloodPressure']],
            'Insulin': [self._dataset[node]['Insulin'], self._dataset[node]['Insulin']]
        }
    ...
    def toString(self):
        ...
        for count in range(0, len(self._nodes)):
            ...
            BloodPressure = self._genRangeFeatures['BloodPressure']
            if BloodPressure[0] == BloodPressure[1]:
                out_string += str(BloodPressure[0]) + ", "
            else:
                out_string += "[" + str(BloodPressure[0]) + " - " + str(BloodPressure[1]) + "], "

            Insulin = self._genRangeFeatures['Insulin']
            if Insulin[0] == Insulin[1]:
                out_string += str(Insulin[0]) + " \n"
            else:
                out_string += "[" + str(Insulin[0]) + " - " + str(Insulin[1]) + "] \n"


            

         #   out_string += self._genCatFeatures['Age'] + ", "
          #  out_string += self._genCatFeatures['BloodPressure'] + ", "
           # out_string += self._genCatFeatures['Insulin'] + "\n"
